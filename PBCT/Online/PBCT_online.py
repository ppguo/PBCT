...
def PBCT_log(csv_name,L,U,Upper_var1,train_index,test_index,repeated_num,coef_file):
    ...
    for i in range(y_test.shape[0]):
        tmp_y = y_test.iloc[i].to_numpy()*std_labled_y+mean_labled_y
        tmp_pre = Reg.predict([Z_test.iloc[i].to_numpy()])[0]*std_labled_y+mean_labled_y
        tmp_error = (tmp_y.to_numpy()[0] - tmp_pre.to_numpy()[0])
        tmp_percent_err =abs(tmp_error/tmp_y.to_numpy()[0])*100
        tmp_error_square = tmp_error * tmp_error
...
if __name__=="__main__":
    ...
    for sweep_num in range(7,16):
        ...
        for i in range(repeated_num):
            ...
            train_index = random_index[i][:33]
            test_index = random_index[i][33:]
            train_num = len(train_index)
            PBCT_RMSE, PBCT_beta_RMSE,LS_beta_RMSE,LS_lasso_RMSE,LS_elasnet_RMSE = PBCT_log(csv_name,sweep_num,train_num-sweep_num,Upper_var1,train_index,test_index,i,coef_file)
        ...
        df.to_csv('./'+file_name+'/'+csv_name+'_result_PBCT_'+str(sweep_num)+'_'+str(Upper_var1)+'_online.csv',index=False)
    avg_all = pd.DataFrame(all_average)
    median_all = pd.DataFrame(all_median)
    avg_all.to_csv('./'+file_name+'/online_avg_'+str(Upper_var1)+'.csv',index = False)
    median_all.to_csv('./'+file_name+'/online_median_'+str(Upper_var1)+'.csv',index = False)
